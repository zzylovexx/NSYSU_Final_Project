...
namespace EtherCat
{
    public partial class Form1 : Form
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (uNID = 0; uNID < uSlaveNum; uNID++)
                    {
                        ...
                        {
                            nSID = 0;
                            CmbNode.Items.Add("NodeID:"+uNID+" - SlotID:"+nSID);
        ...
        private void TimCheckStatus_Tick(object sender, EventArgs e)
        {
            ...
            ushort uMCDone = 0, uStstus = 0, uRet = 0;
            int nCmd = 0, nPos = 0;
            if (g_nESCExistCards > 0)
            {
        ...
        private void BtnHoming_Click(object sender, EventArgs e)
        {
            ushort uMode = 0;
            int nOffset = 0;
            uint uTacc = 0, nMaxVel = 0, nStrVel = 0;

            nStrVel = Convert.ToUInt16(TxtFirstVel.Text);
            nMaxVel = Convert.ToUInt16(TxtSecondVel.Text);
            uMode = Convert.ToUInt16(TxtMode.Text);
            nOffset = Convert.ToInt32(TxtOffset.Text);
            uTacc = Convert.ToUInt32(TxtDec.Text);

            g_uRet = CEtherCAT_DLL.CS_ECAT_Slave_Home_Config(g_uESCCardNo, g_uESCNodeID, g_uESCSlotID, uMode
                , nOffset, nStrVel, nMaxVel, uTacc);
            if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
            {
            ...
            {
                g_uRet = CEtherCAT_DLL.CS_ECAT_Slave_Home_Move(g_uESCCardNo, g_uESCNodeID, g_uESCSlotID);
                if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
                {
        ...
        private void BtnStop_Click(object sender, EventArgs e)
        {
            ...
            dTdec = Convert.ToDouble(TxtDec.Text);
