...
namespace EtherCat
{
    public partial class Form1 : Form
    {
        ...
        private void TimCheckStatus_Tick(object sender, EventArgs e)
        {
            ...
            if (g_nESCExistCards > 0)
            {
                if (ChkRealTimeUpdate.Checked == true)
                {
                    ...
                    uRet = CEtherCAT_DLL.CS_ECAT_Slave_Motion_Get_Current_Speed(g_uESCCardNo, g_uESCNodeID, g_uESCSlotID, ref nSpeed);
                    if (uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
                    {
                        AddErrMsg("ECAT_Slave_Motion_Get_Current_Speed Fail, Error Code :" + uRet.ToString());
                        ChkRealTimeUpdate.Checked = false;
                        return;
                    }
        ...
        {            
            ...
            if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
            {
                AddErrMsg("_ECAT_Slave_Motion_Set_Svon, Error Code = "+ g_uRet.ToString(), true);
                if (uCheckOnOff == 1)
                    RdoSVON.Checked = true;
                else
                    RdoSVOFF.Checked = true;
            }
        }
        ...
        private void BtnResetAlarm_Click(object sender, EventArgs e)
        {
            ...
            if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
            {
                AddErrMsg("_ECAT_Slave_Motion_Ralm, ErrorCode = " + g_uRet.ToString());
            }
        }
        ...
        private void BtnResetStatus_Click(object sender, EventArgs e)
        {
            ...
            if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
            {
                AddErrMsg("_ECAT_Slave_Motion_Set_Position, ErrorCode = " + g_uRet.ToString(), true);
            }
            g_uRet = CEtherCAT_DLL.CS_ECAT_Slave_Motion_Set_Command(g_uESCCardNo, g_uESCNodeID, g_uESCSlotID, 0);
            if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
            {
                AddErrMsg("_ECAT_Slave_Motion_Set_Command, ErrorCode = " + g_uRet.ToString(), true);
            }
        }
        ...
        {
            int nTargetVel = 0;
            uint uAcceleration = 0, uDeceleration = 0;

            nTargetVel = Convert.ToInt32(TxtTargetVel.Text); // 目標速度，(0x60FF Sub 0) inc代表的是驅動器內部設定之單位值
            ...
            if (nDir == 0)
                nTargetVel = (short)(0 - nTargetVel);

            g_uRet = CEtherCAT_DLL.CS_ECAT_Slave_PV_Start_Move(g_uESCCardNo, g_uESCNodeID, g_uESCSlotID, nTargetVel, uAcceleration, uDeceleration);

            if (g_uRet != CEtherCAT_DLL_Err.ERR_ECAT_NO_ERROR)
            {
                AddErrMsg("CS_ECAT_Slave_PV_Start_Move, ErrorCode = " + g_uRet.ToString(), true);
            }
        }
